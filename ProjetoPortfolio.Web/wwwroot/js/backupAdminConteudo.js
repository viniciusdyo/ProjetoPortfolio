...
const adminConteudo = () => {
    ...
    const listarConteudos = async () => {
        ...
        conteudos.forEach(i => {
            ...
            conteudoRoot.appendChild(divCard);
        ...
        conteudos.forEach(c => {
            ...
            var modalRemover = criarModalRemover(c);
            main.innerHTML += modalRemover;
            excluirConteudoForm()

            c.ativosConteudo.forEach(e => {
                var ativo = `<a class="btn btn-danger btn-remover-ativo text-white my-3">Remover</a>
                ...
                var divAtivo = document.createElement('div');
                divAtivo.classList.add('form-ativo', 'col-12');
                divAtivo.innerHTML += ativo;
                divInputAtivos.appendChild(divAtivo);
        ...
        salvarEdicaoConteudo()

        // modalEditarSubmit();
        // excluirConteudoForm();
    };

    const salvarEdicaoConteudo = function () {
        ...
        formEditar.forEach(form => {
            ...
            modalEditarSubmit(form, btnSalvar)
        })
    };
    ...
    const adicionarAtivo = () => {
        ...
        btnAdicionar.forEach(b => {
            ...
            b.addEventListener('click', e => {
                e.preventDefault();
                divAtivos.appendChild(divAtivo);
                removerAtivo();
            });
    ...
    const modalEditarSubmit = (form, btnSalvar) => {
        var ativosArr = [];
        var count = 0;
        var ativo = {};
        console.log(form)
    ...
    const criarModalAdicionar = () => {
        ...
        <div class="modal-dialog">
            <div id="editar-root" class="modal-content">
                ...
                <div class="modal-body">
                    <form id="adicionar-conteudo-form" data-bs-theme="portfolio-dark" class="row row-cols-1 g-3 form-adicionar-conteudo " autocomplete="off">
                        <div class="text-danger"></div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <a id="${conteudo.nomeNormalizado}-remover-submit" class="btn btn-danger text-white btn-remover-conteudo"> Excluir</a>
            </div>
          </div>
        </div>
      </div>
    ...
    const excluirConteudoForm = () => {
        var btnExcluir = document.querySelectorAll(".btn-remover-conteudo");
        console.log(btnExcluir)
        btnExcluir.forEach(btn => {
            console.log(btn)
            var btnParent = btn.parentElement;
            var parent = btnParent.parentElement;
            var form = parent.querySelector('form');
            console.log(form);
            var formData = new FormData(form);
            var id = String(formData.get('id'));
